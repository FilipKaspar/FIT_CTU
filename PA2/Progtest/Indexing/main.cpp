...
public:
    CIndex(const T_ & srcDat, const C_ & srcComp = C_()) : comparator(srcComp) {
        data = srcDat;
    };

    set<size_t> search (const T_ & searchedText) const{
        set<size_t> foundPositions;
        bool found = true;

        size_t pos = 0;
        for(auto dataItr = data.begin(); dataItr != data.end(); dataItr++){
            auto tempItr = dataItr;
            for(auto searchedTextItr = searchedText.begin(); searchedTextItr != searchedText.end(); searchedTextItr++){
                if(tempItr == data.end()){
                    found = false;
                    break;
                }
                if(!(comparator(*tempItr, *searchedTextItr)) && !(comparator(*searchedTextItr, *tempItr))){
                    tempItr++;
                    found = true;
                } else {
                    found = false;
            ...
            if(found){
                foundPositions.insert(pos);
            }
            pos++;
        }

        /*for(auto i : foundPositions){
            cout << i << endl;
        }*/
